javascript:var b64pad='';var chrsz=8;var inputArray=new Array();var specialIterations=[{domains:['fh-wels.at','fh-ooe.at','fhooe.at'],iter:4,addOnes:true},{domains:['cardcomplete.com'],iter:1,addOnes:true},{domains:['google.com','google'],iter:1,addOnes:false},{domains:['evernote.com'],iter:1,addOnes:false}];function b64_sha1(s){return binb2b64(core_sha1(str2binb(s),s.length*chrsz));}
...
if(!flagTypeInDomain){var Domain=window.prompt('Enter your domain');flagShow=true;if(Domain!=null&&Domain!=''){domain=Domain;flagTypeInDomain=true;}}
for(i=0;i<specialIterations.length;i++){for(j=0;j<specialIterations[i].domains.length;j++){if((domain.indexOf(specialIterations[i].domains[j])!=-1)){domain=specialIterations[i].domains[0]+specialIterations[i].iter;var addOnes=specialIterations[i].addOnes;}}}
...
flagWritePWN=window.prompt(domain,p);if(flagShow){if(flagWritePWN){D.value=p;D.focus();D.style.background='#FF69B4';wrotePWAtLeastOnce=true;}}else{D.value=p;D.focus();D.style.background='#FF69B4';wrotePWAtLeastOnce=true;}}}
