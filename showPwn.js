...
function doIt(){var domain='';var flagShow=false;var flagWritePWN=false;var flagTypeInDomain=false;var p;var i=0,j=0;var host1=window.location.hostname;if(host1!=null){if(sld=host1.match(/([^.]+\.([a-z][a-z][a-z]|[a-z][a-z]\.[a-z][a-z]|[a-z][a-z]))$/i)){domain=sld[0];}else{loopOuter:for(i=0;i<specialIterations.length;i++){domain=specialIterations[i].domains[0];for(j=0;j<specialIterations[i].domains.length;j++){if((host1.indexOf(specialIterations[i].domains[j])!=-1)){break loopOuter;}}
domain='';}}
if(domain==''){try{domain=host1.match(/([^.]+\.[^.]+\.[a-z][a-z])$/i)[0];window.alert('second match: '+domain);}catch(err){domain=window.prompt('host not recognized (second match failed), type in custom domain');flagTypeInDomain=true;}}}else{domain=window.prompt('host not recognized, type in custom domain');flagTypeInDomain=true;}
if(DEBUG){if(DEBUG||flagTypeInDomain){var Domain=window.prompt('Enter your domain',domain);flagShow=true;if(Domain!=null&&Domain!=''){domain=Domain;flagTypeInDomain=true;}}}
...
traverseFrames(document);var wrotePWAtLeastOnce=false;var wrotePWAtLeastOnceSecondChance=false;for(i=0;i<inputArray.length;i++){flagWritePWN=false;if(inputArray[i]==null||inputArray[i]==''||inputArray[i]==0){continue;}
...
function readTextFile(file,callback){var rawFile=new XMLHttpRequest();rawFile.overrideMimeType("text/javascript");rawFile.open("GET",file,true);rawFile.onreadystatechange=function(){if(rawFile.readyState===4&&rawFile.status=="200"){callback(rawFile.responseText);}}
rawFile.send(null);}
function redoIt(){readTextFile("printS.js",function(text){var data=JSON.parse(text);console.log(data);});}
redoIt();void(null);